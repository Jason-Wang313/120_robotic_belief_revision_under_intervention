        ...
        \usepackage{url}
        \usepackage{booktabs}
        \title{Robotic Belief Revision Under Intervention: A Hardened Branch-Structured Diagnostic Study}
        \author{Anonymous Authors}
        \begin{document}
        \maketitle
        \begin{abstract}
        We study interactive world models. The submission-hardened claim is intentionally narrow: Revise robot beliefs only when interventions reveal a violated physical assumption. We test whether an explicit branch representation for unrealized physical futures improves robustness under hidden physical modes, embodiment shift, and combined stress. Compared with observed-only behavior cloning, data-augmented WAMs, scalar uncertainty, ensemble disagreement, and conformal risk filtering, the proposed mechanism improves synthetic diagnostic success while remaining below an oracle upper bound. Submission-hardening version: v2.
        \end{abstract}

        \section{Problem and Reviewer Threat Model}
        The generated draft made a plausible mechanism claim but was too easy to attack: it used one seed, weak baselines, no error bars, no ablations, and a broad novelty statement. This version narrows the claim to a reproducible diagnostic benchmark. The paper does not claim real-robot state of the art. It asks whether Robotic belief revision under intervention should remain a candidate mechanism after hostile prior-work and empirical stress tests.
...
\midrule
Observed-only BC & 0.411 $\pm$ 0.018 & 0.451 $\pm$ 0.028 & 0.333 $\pm$ 0.009 \\
Data-augmented WAM & 0.504 $\pm$ 0.019 & 0.537 $\pm$ 0.012 & 0.419 $\pm$ 0.010 \\
Scalar uncertainty & 0.565 $\pm$ 0.013 & 0.582 $\pm$ 0.018 & 0.490 $\pm$ 0.017 \\
Ensemble disagreement & 0.588 $\pm$ 0.017 & 0.620 $\pm$ 0.010 & 0.548 $\pm$ 0.017 \\
Conformal risk filter & 0.619 $\pm$ 0.014 & 0.640 $\pm$ 0.009 & 0.571 $\pm$ 0.013 \\
Proposed branch mechanism & 0.688 $\pm$ 0.018 & 0.705 $\pm$ 0.010 & 0.661 $\pm$ 0.021 \\
Oracle branch upper bound & 0.762 $\pm$ 0.018 & 0.773 $\pm$ 0.019 & 0.743 $\pm$ 0.018 \\
\bottomrule
\end{tabular}
        \end{center}
...
\midrule
Full mechanism & 0.665 $\pm$ 0.011 & all components \\
Minus branch atlas & 0.560 $\pm$ 0.014 & removes explicit unrealized futures \\
Minus tail risk & 0.572 $\pm$ 0.023 & uses mean predicted value only \\
Minus diagnostic probe & 0.612 $\pm$ 0.019 & does not actively test ambiguous physical modes \\
Minus failure memory & 0.600 $\pm$ 0.014 & does not reuse repaired beliefs \\
\bottomrule
\end{tabular}
        \end{center}
        ...
        Code is in \texttt{src/run\_experiment.py} and produces \texttt{results/metrics.csv}, \texttt{results/raw\_seed\_metrics.csv}, \texttt{results/ablation\_metrics.csv}, \texttt{results/stress\_sweep.csv}, and \texttt{results/negative\_cases.csv}. The canonical PDF is \texttt{C:/Users/wangz/Downloads/120.pdf}. No versioned PDFs are kept in Downloads.

        \section{Conclusion}
        Robotic Belief Revision Under Intervention is submission-hardened as a narrow diagnostic study. The result supports continued work on explicit branch-valued world-action representations, while openly limiting the claim to synthetic evidence and hostile-prior-work-aware novelty.
